...
const location6 = new sourcegraph.Location(new URL('http://test/2'), range2) // overlapping URI
const location7 = new sourcegraph.Location(new URL('http://test/3'), range2) // overlapping URI
const location8 = new sourcegraph.Location(new URL('http://test/4'), range2) // overlapping URI
...
describe('createReferencesProvider', () => {
    ...
    it('supplements LSIF results with non-overlapping search results', async () => {
        const result = createReferencesProvider(
            ...
            () => asyncGeneratorFromValues([[location4], [location4, location6, location7, location8]])
        ).provideReferences(textDocument, position, {
            includeDeclaration: false,
        }) as Observable<sourcegraph.Badged<sourcegraph.Location>[]>
