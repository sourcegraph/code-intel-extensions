...
export const canGenerateTraceUrl = (
    val: any
): val is { generateTraceURL(): string } =>
    typeof val === 'object' &&
    val !== null &&
    typeof val.generateTraceURL === 'function'
...
export async function tracePromise<T>(
    ...
    const span = tracer.startSpan(
        operationName,
        childOfOrOptions instanceof Span ||
            childOfOrOptions instanceof SpanContext
...
export async function sendTracedRequest<P, R>(
    ...
    params: P,
    {
        ...
        tracer,
        token,
    }: { span: Span; tracer: Tracer; token: CancellationToken }
): Promise<R> {
