...
export async function* findPackageDependentsWithNpm(
    ...
    span.setTag('packageName', packageName)
    logger.log(
        `Searching for dependents of package "${packageName}" through npm`
    )
    const limit = 100
    ...
    for (let skip = 0; true; skip += limit) {
        ...
        if (result.rows.length === 0) {
            logger.log(
                `Found ${seenRepos.size} dependent repos of "${packageName}"`
            )
            return
        }
        for (const row of result.rows) {
            ...
            try {
                ...
                const repoName = await resolvePackageNameToRepoName(
                    packageMeta,
                    sgEndpoint,
                    { tracer, span }
                )
...
export async function* findPackageDependentsWithSourcegraphSearch(
    ...
    const results = await search(
        `file:package.json$ ${packageName} max:1000`,
        sgEndpoint,
        { span, tracer }
    )
    const seenRepos = new Set<string>()
...
export async function* findPackageDependentsWithSourcegraphExtensionRegistry(
    ...
    logger.log(
        `Searching for dependents to "sourcegraph" through Sourcegraph extension registry`
    )
    ...
    for (const extension of extensions) {
        try {
            ...
            logErrorEvent(span, err)
            logger.error(
                `Error mapping extension "${extension.extensionID}" to Sourcegraph repo`,
                err
            )
        }
    }
...
export async function findClosestPackageJson(
    ...
    return await tracePromise(
        ...
        async (span): Promise<[URL, PackageJson]> => {
            ...
            while (true) {
                if (
                    !parent.href.startsWith(rootUri.href) ||
                    parent.href === rootUri.href
                ) {
                    throw new Error(
                        `No package.json found for ${resource} under root ${rootUri}`
                    )
                }
                const packageJsonUri = new URL('package.json', parent.href)
                const response = await tracedFetch(packageJsonUri, {
                    headers,
                    tracer,
                    span,
                })
...
export async function findPackageName(
    ...
    if (!packageJson.name) {
        throw new Error(
            `package.json at ${packageJsonUrl} does not contain a name`
        )
    }
...
export async function resolvePackageNameToRepoName(
    ...
    const repoName = await resolveRepository(cloneUrl, sgEndpoint, {
        span,
        tracer,
    })
    return repoName
