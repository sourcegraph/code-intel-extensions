 ...
 */
export function resolveServerRootUri(
    textDocumentUri: URL,
    ...
    rootUri.pathname =
        [
            textDocumentUri.host + textDocumentUri.pathname,
 ...
 */
export function toServerTextDocumentUri(
    textDocumentUri: URL,
    sgEndpoint: SourcegraphEndpoint
): URL {
    ...
    const serverTextDocumentUri = new URL(
        textDocumentUri.hash.substr(1),
        rootUri.href
    )
...
export function toSourcegraphTextDocumentUri(serverTextDocumentUri: URL): URL {
    const { repoName, rev, filePath } = parseSourcegraphRawUrl(
        serverTextDocumentUri
    )
...
export function parseSourcegraphRawUrl(rawUrl: URL): RawUrl {
    const match = rawUrl.pathname.match(
        /^\/([^@]+)(?:@([^\/]+))?\/-\/raw\/(.*)$/
    )
    if (!match) {
