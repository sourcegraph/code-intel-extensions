...
type LanguageSpec = { handlerArgs: HandlerArgs; stylized: string }
...
const languages: { [name: string]: LanguageSpec } = {
    java: { handlerArgs: { fileExts: ['java'] }, stylized: 'Java' },
    cpp: { handlerArgs: { fileExts: ['c', 'cc', 'cpp', 'hh', 'h'] }, stylized: 'C++' },
    ruby: {
        handlerArgs: {
            fileExts: [
                'rb',
    ...
    php: {
        handlerArgs: { fileExts: ['php', 'phtml', 'php3', 'php4', 'php5', 'php6', 'php7', 'phps'] },
        stylized: 'PHP',
    },
    csharp: { handlerArgs: { fileExts: ['cs', 'csx'] }, stylized: 'C#' },
    shell: { handlerArgs: { fileExts: ['sh', 'bash', 'zsh'] }, stylized: 'Shell' },
    scala: {
        handlerArgs: {
    ...
    elixir: {
        handlerArgs: { fileExts: ['ex', 'exs'], definitionPatterns: ['\\b(def|defp|defmodule)\\s%s\\b'] },
        stylized: 'Elixir',
    },
    perl: {
        handlerArgs: { fileExts: ['pl', 'al', 'cgi', 'fcgi', 'perl', 'ph', 'plx', 'pm', 'pod', 'psgi', 't'] },
        stylized: 'Perl',
    },
    lua: { handlerArgs: { fileExts: ['lua', 'fcgi', 'nse', 'pd_lua', 'rbxs', 'wlua'] }, stylized: 'Lua' },
    clojure: { handlerArgs: { fileExts: ['clj', 'cljs', 'cljx'] }, stylized: 'Clojure' },
    haskell: {
        handlerArgs: {
            fileExts: ['hs', 'hsc'],
            definitionPatterns: [
    ...
    powershell: { handlerArgs: { fileExts: ['ps1', 'psd1', 'psm1'] }, stylized: 'PowerShell' },
    lisp: {
        handlerArgs: { fileExts: ['lisp', 'asd', 'cl', 'lsp', 'l', 'ny', 'podsl', 'sexp', 'el'] },
        stylized: 'Lisp',
    },
    erlang: { handlerArgs: { fileExts: ['erl'] }, stylized: 'Erlang' },
    dart: { handlerArgs: { fileExts: ['dart'] }, stylized: 'Dart' },
    ocaml: {
        handlerArgs: { fileExts: ['ml', 'eliom', 'eliomi', 'ml4', 'mli', 'mll', 'mly', 're'] },
        stylized: 'OCaml',
    },
...
function main(): void {
    ...
    _.forEach(_.pickBy(languages, languageFilter), ({ handlerArgs, stylized }: LanguageSpec, name) => {
